...
list_vocab = jieba.lcut(list1)
# print(words[i])
list_pos = list(postagger.postag(list_vocab))
# print(words_class[i])
...
with torch.no_grad():
    ...
    print(training_data[0][0])
    idxs = model(precheck_sent)[1]  # tuple (tensor ,list)
    pre = ix_sequence_to_tag_sequence(idxs, ix_to_tag)
    print(pre)
...
def getR(pre, stwords):
    Rs = []
    for i in range(len(ix_to_tag)):
        TP = 0
        lens = 0
        if ix_to_tag[i] not in stwords:
            continue
        tp = 0
        for k in range(len(pre)):
            if ix_to_tag[i] == stwords[k]:
                lens += 1
                if pre[k] == stwords[k]:
                    tp = tp + 1
        TP = TP + tp
...
P = getP(pre, list_pos)
F = getF(P, R)
fp = open('bilstm+crf词性标注.txt', 'w', encoding='utf-8')
for word in pre:
    fp.write(word + ',')
fp.close()
print('BiLSTM_CRF模型训练与预测共耗时:{}'.format(over-start))
